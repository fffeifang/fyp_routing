...
def setup():
	...
	nodes = []
	with open('traces/allnodes_backup.txt', 'r') as f:
	...
	G = nx.DiGraph()
	listC = []
	with open('traces/channels_backup.txt', 'r') as f:
		for line in f: 
			if 'source' in line: 
				source = line.split()[1]
	...
	edges_to_remove = [(u, v) for (u, v) in G.edges if (v, u) not in G.edges]
	...
	trans = []
	with open('traces/BitcoinVal.txt', 'r') as f: 
		for line in f:
